  ...
  return (
    <div className="animate-slide-up-banner" style={{
      ...
      <div style={{
        ...
        background: dark
          ? '#1E293B'
          : (isCorrect ? 'linear-gradient(180deg, #E8F5E9 0%, #C8E6C9 100%)' : 'linear-gradient(180deg, #FFEBEE 0%, #FFCDD2 100%)'),
        borderTop: dark
          ? (isCorrect ? '3px solid #10B981' : '3px solid #EF4444')
          : (isCorrect ? '4px solid #4CAF50' : '4px solid #EF5350'),
        ...
        boxShadow: dark
          ...
          : (isCorrect ? '0 -8px 32px rgba(76,175,80,0.15)' : '0 -8px 32px rgba(244,67,54,0.12)'),
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
          {isCorrect
            ? <CheckCircle size={32} weight="fill" color={dark ? '#10B981' : '#4CAF50'} />
        ...
        <p style={{
          ...
          <PhraseIcon name={phrase.emoji} size={16} color={dark ? '#94A3B8' : (isCorrect ? '#388E3C' : '#D32F2F')} /> {phrase.text}
        </p>
        {autoAdvance ? (
          <div style={{
            width: '100%',
            height: 6,
            borderRadius: 3,
            background: dark ? 'rgba(255,255,255,0.1)' : 'rgba(76,175,80,0.15)',
            overflow: 'hidden',
            marginTop: 8,
          }}>
            <div style={{
          ...
          <button onClick={onContinue} style={{
            ...
            background: dark
              ...
              : (isCorrect ? 'linear-gradient(180deg, #5CBF60 0%, #4CAF50 40%, #45A049 100%)' : 'linear-gradient(180deg, #EF5350 0%, #F44336 50%, #E53935 100%)'),
            borderBottom: dark
              ? (isCorrect ? '3px solid #047857' : '3px solid #B91C1C')
              : (isCorrect ? '3px solid #2E7D32' : '3px solid #C62828'),
            ...
            onPointerDown={(e) => {
              ...
              e.currentTarget.style.borderBottom = isCorrect
                ? (dark ? '1px solid #047857' : '1px solid #2E7D32')
                : (dark ? '1px solid #B91C1C' : '1px solid #C62828');
            }}
            onPointerUp={(e) => {
              e.currentTarget.style.transform = 'translateY(0)';
              e.currentTarget.style.borderBottom = isCorrect
                ? (dark ? '3px solid #047857' : '3px solid #2E7D32')
