...
export default function Progress() {
  ...
  return (
    <div style={{ background: '#F7F8FA', minHeight: '100dvh' }}>
      {/* Green top bar */}
      <div style={{ background: 'linear-gradient(180deg, #56C45A 0%, #4CAF50 40%, #3D9142 100%)', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10, minHeight: 48, boxShadow: '0 4px 20px rgba(56,142,60,0.3)' }}>
      ...
      <div style={{ padding: '20px 16px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
          ...
          <StatCard icon={CheckCircle} iconColor="#4CAF50" iconBg="#E8F5E9" label={t('progress.lessons_done')} value={`${lessonsCompleted.length}/10`} />
        </div>

        {/* Growth Area */}
        <p style={{ fontSize: 15, fontWeight: 700, color: '#333', fontFamily: 'Nunito, sans-serif', marginBottom: 14, marginTop: 8 }}>
...
function StatCard({ icon: Icon, iconColor, iconBg, label, value }) {
  return (
    <div style={{ background: 'linear-gradient(180deg, #FFFFFF 0%, #FCFCFC 100%)', borderRadius: 16, padding: '16px 14px', border: '1px solid rgba(0,0,0,0.05)', borderBottom: '3px solid rgba(0,0,0,0.06)', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
      ...
      <span style={{ fontSize: 20, fontWeight: 800, color: '#333', fontFamily: 'Nunito, sans-serif' }}>{value}</span>
      <span style={{ fontSize: 11, fontWeight: 600, color: '#9E9E9E', fontFamily: 'Nunito, sans-serif', textAlign: 'center' }}>{label}</span>
    </div>
  );
}
    ...
    <div style={{
      ...
      <div style={{ position: 'relative', width: 86, height: 86 }}>
        <svg width="86" height="86" viewBox="0 0 86 86">
          ...
          <circle cx="43" cy="43" r={radius} fill="none" stroke="#EEEEEE" strokeWidth={stroke} />
        ...
        <div style={{
          ...
          <span style={{ fontSize: 18, fontWeight: 800, color: '#333', fontFamily: 'Nunito, sans-serif' }}>
            {percentage}%
          </span>
        </div>
      </div>
      <span style={{
        fontSize: 11, fontWeight: 600, color: '#757575', fontFamily: 'Nunito, sans-serif',
